...
from clispy import environment
...
class UnitTestCase(unittest.TestCase):
    def testEnv_parse_rest_argument(self):
        params = ['x', '&REST', 'y']
        args = [1, 2, 3, 4, 5]
        params, args = environment._Environment._parse_rest_argument(params, args)
    ...
    def testEnvironment(self):
        param = Symbol('a')
        args = [2, 3, 4]
        test_env = environment._Environment(param, args)
        ...
        param = ['x', 'y']
        args = [2]
        self.assertRaisesRegex(TypeError, "expected .*, given .*", environment._Environment, param, args)
        ...
        param = ['a']
        args = [1]
        sub_test_env = environment._Environment(param, args, outer=test_env)
        self.assertEqual(sub_test_env.find('a'), sub_test_env)
        self.assertEqual(sub_test_env.find('x'), test_env)

        params = ['x', '&REST', 'y']
        args = [1, 2, 3, 4, 5]
        test_env = environment._Environment(params, args)
